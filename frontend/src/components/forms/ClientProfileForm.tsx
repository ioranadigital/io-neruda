...
export default function ClientProfileForm({ client: initialClient, onSubmit, isLoading: externalIsLoading }: ClientProfileFormProps) {
  ...
  // Sync formData when initialClient changes (e.g., after update from database)
  React.useEffect(() => {
    console.log('🔍 useEffect triggered, initialClient:', initialClient?.id);
    if (initialClient) {
      console.log('📥 Loading client from initialClient, setting isDirty to false');
      setFormData(initialClient);
      setIsDirty(false);
    }
  }, [initialClient?.id, initialClient?.updated_at]);
  ...
  const handleChange = (field: string, value: any) => {
    console.log('✏️ handleChange:', field, 'isDirty will be true');
    setFormData(prev => ({
      ...prev,
      [field]: value,
    }));
  ...
  const handleSave = async () => {
    ...
    console.log('initialClient:', initialClient?.id);
    console.log('onSubmit exists:', !!onSubmit);

    try {
      setIsSaving(true);
      if (onSubmit) {
        console.log('🔄 Using onSubmit (new client or custom)');
        await onSubmit(formData);
      } else if (initialClient && initialClient.id) {
        console.log('🔄 Using updateClientBrandMemory (existing client)');
        await updateClientBrandMemory(initialClient.id, formData);
        setIsDirty(false);
        showToast.success('✅ Cliente guardado exitosamente');
      } else {
        console.error('❌ No way to save: no onSubmit and no initialClient.id');
        throw new Error('No se puede guardar sin initialClient.id o onSubmit');
      }
    } catch (err) {
  ...
  return (
    <div className="bg-white rounded-lg shadow-xl overflow-hidden flex flex-col h-full w-full">
      {/* Tab Navigation */}
      <div className="border-b border-slate-200 flex-shrink-0 overflow-hidden" style={{ backgroundColor: '#f4fbf7' }}>
          ...
          {TABS.map(tab => (
            <button
              ...
              style={activeTab === tab.key
                ? { borderColor: '#4aa87a', color: '#4aa87a', backgroundColor: 'white' }
                : {}}
            >
              {tab.icon}
      ...
      {/* Footer Actions */}
      {isDirty && (
        <div className="border-t border-slate-200 p-4 flex gap-3 justify-end sticky bottom-0 bg-white">
          <button
            onClick={handleCancel}
            disabled={isSaving}
            className="px-5 py-2 border border-slate-300 text-slate-600 rounded-lg hover:bg-slate-50 transition disabled:opacity-50 font-medium text-sm flex items-center gap-2"
          >
            <X size={16} />
            Cancelar
          </button>
          <button
            onClick={handleSave}
            disabled={isSaving}
            className="px-5 py-2 text-white rounded-lg hover:opacity-90 transition disabled:opacity-50 font-medium text-sm flex items-center gap-2"
