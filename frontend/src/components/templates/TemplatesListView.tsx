...
import { TemplateGrid, ICON_MAP } from './TemplateGrid';
...
export default function TemplatesListView() {
  ...
  const [enabledInStep1, setEnabledInStep1] = useState<Set<string>>(new Set());

  // Load enabled templates from localStorage on mount
  React.useEffect(() => {
    const saved = localStorage.getItem('templates-step1-enabled');
    if (saved) {
      try {
        setEnabledInStep1(new Set(JSON.parse(saved)));
      } catch (err) {
        console.error('Failed to load step1 templates preference:', err);
      }
    }
  }, []);

  const toggleStep1 = (templateId: string) => {
    setEnabledInStep1((prev) => {
      ...
      localStorage.setItem('templates-step1-enabled', JSON.stringify(Array.from(next)));
      return next;
    });
  };
