...
test('Test dual AI model buttons in PASO 7', async ({ page }) => {
  ...
  try {
    const intentBtn = page.locator('button:has-text("Educacional")').first();
    await intentBtn.click({ timeout: 2000 });
    await page.waitForTimeout(300);
  } catch (e) {
    console.log('Intent button not found, continuing...');
  }

  // Select Tone
  try {
    const toneBtn = page.locator('button:has-text("Profesional")').first();
    await toneBtn.click({ timeout: 2000 });
    await page.waitForTimeout(300);
  } catch (e) {
    console.log('Tone button not found, continuing...');
  }
  ...
  try {
    const formatBtn = page.locator('button:has-text("Blog")').first();
    await formatBtn.click({ timeout: 2000 });
    await page.waitForTimeout(300);
  } catch (e) {
    console.log('Format button not found, continuing...');
  }
